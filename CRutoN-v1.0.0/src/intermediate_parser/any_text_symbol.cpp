...
Automaton* AnyTextSymbol::getCopy()
{
	AnyTextSymbol* new_any_text_symbol = new AnyTextSymbol();
	new_any_text_symbol->setRepeat(repeat);
	return new_any_text_symbol;
}

const bool AnyTextSymbol::isAccepting() const
{
...
void AnyTextSymbol::feedToken(const std::string token)
{
...
const bool AnyTextSymbol::wouldBeDead(const std::string token)
{
	return (repeat == Automaton::REPEAT_NONE && num_repeats == 1)
		|| (repeat != Automaton::REPEAT_ONE_OR_MORE && num_repeats == repeat);
}

void AnyTextSymbol::resetSymbol()
{
	is_dead = false;
	num_repeats = 0;
	input = "";
