...
Automaton* IntegerSymbol::getCopy()
{
	IntegerSymbol* new_integer_symbol = new IntegerSymbol();
	new_integer_symbol->setRepeat(repeat);
	return new_integer_symbol;
}

const bool IntegerSymbol::isAccepting() const
{
...
void IntegerSymbol::feedToken(const std::string token)
{
...
const bool IntegerSymbol::wouldBeDead(const std::string token)
{
...
void IntegerSymbol::resetAutomata()
{
	num_repeats = 0;
}

void IntegerSymbol::resetSymbol()
{
	is_dead = false;
	num_repeats = 0;
	input = "";
